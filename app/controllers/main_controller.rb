class MainController < ApplicationController
  ...
  def index
    if params[:search].present?
      @posts = Post.where("title ILIKE ?", "%#{params[:search]}%").order('posts.date DESC').includes(:tags).paginate(:page => params[:page], :per_page => 5)
    else
  ...
  def by_date
    # get the post(s) for that date
    if params[:slug]
      @post = Post.where("slug = ?", params[:slug]).first
    else
      if params[:day]
        @posts = Post.where('posts.date = ?', "#{params[:year]}-#{params[:month]}-#{params[:day]}")
      elsif params[:month]
        @posts = Post.where("date_part('year', posts.date) = ? AND date_part('month', posts.date) = ?", params[:year], params[:month])
      elsif params[:year]
        @posts = Post.where("date_part('year', posts.date) = ?", params[:year])
      else
  ...
  private
    ...
    def build_archive_links
      ...
      @results = Post.where("date_part('year', date) = ?",  @cur_year).order('date DESC')
